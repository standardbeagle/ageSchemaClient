/**
 * Fixed Organization Chart Example Integration Test
 *
...
// Test configuration
const TEST_GRAPH = 'fixed_org_chart';
const TEST_SCHEMA = `test_${randomBytes(3).toString('hex')}`;

describe('Fixed Organization Chart Example', () => {
  ...
  beforeAll(async () => {
    ...
    // Set search path
    await queryExecutor.executeSQL(`SET search_path TO ${TEST_SCHEMA}, ag_catalog, public`);
